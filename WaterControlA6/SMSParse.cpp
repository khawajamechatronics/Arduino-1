...
//  Serial.println(tempbuf);
  ...
  Serial.println(tempbuf);
 // HTTPGET(EEPROMGetIndex(HTTPs),tempbuf,80);
}
/*\
 *   Possible messages
 *   b boot
...
void SMSParse(char * s)
{
 ...
 //     sprintf(tempbuf,"%s/RawData.php?AC=tapchange",EEPROMGetIndex(WWWPATH));
 //     strcpy(tempbuf,EEPROMGetIndex(WWWPATH));
   //   strcat(tempbuf,"/RawData.php?AC=tapchange&imei=");
     // strcat(tempbuf,IMEI);
   //   strcat(tempbuf,"&tap=tapclose&epoch=123");
      Serial.println(tempbuf);
      tcpActive = HTTPGET(EEPROMGetIndex(HTTPs),tempbuf,80);
      break;
    case 'O':
