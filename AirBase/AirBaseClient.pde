#ifdef LOCAL_UDP
byte smcServer[6] = {192, 168, 1, 107,0,0};
#else
byte smcServer[6];
#endif
ASocket SMCDataSocket;
...
bool AirBaseClientInit()
{
  ...
  if (result == true)
  {
    SMCDataSocket.initUDP(4321);  //source port of 0 means one will be auto-picked
    SMCDataSocket.beginPacketUDP(smcServer,SMCDataUdpPort);  //Start a new UDP packet for
