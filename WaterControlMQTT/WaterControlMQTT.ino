...
void Analyze()
{
  if (CurrentMeterCount[0] == 0) // not flowing
  {
  ...
  {
    if (PreviousMeterCount[0] == 0) // just started
    {
 ...
 //     lasttap = TAP_OPEN;
    ...
    else
    {
      if ((millis()/ONE_SECOND - StartCountingPeriod) >= MAX_FLOW_TIME)  // running for a minute or more)
      {
        ...
        sprintf(tempbuf,"/WaterControl/RawData.php?action=leak&imei=%s&uptime=%lu&meterticks=%lu&liters=%lu&tap=%s&rawts=%s",
          IMEI,millis()/1000,metercount[0],metercount[0]/TicksPerLiter,TapToText(),RemoteGetClock());
  ...
  PreviousMeterCount[0] = CurrentMeterCount[0];
  CurrentMeterCount[0] = 0;
}
...
void setup()
{
  ...
  RGBInit();
  pinMode(METER1_DRDY,INPUT);
  pinMode(METER2_DRDY,INPUT);
...
// force relay to power off position
  ...
  attachInterrupt(digitalPinToInterrupt(METER1_DRDY), Meter1Count, RISING);
  attachInterrupt(digitalPinToInterrupt(METER2_DRDY), Meter2Count, RISING);
  TapChangeState(TAP_OPEN);
  // startup the GSM , can take time
  remoteAvailable = RemoteInit(MQTT_REMOTE);
