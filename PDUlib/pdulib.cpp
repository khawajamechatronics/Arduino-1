...
bool PDU::setRecepient(char *rec)
{
  bool rc = false;
  if (strlen(rec) < MAX_NUMBER_LENGTH)
  {
    strcpy(recbuff, rec);
...
bool PDU::setMessage(char *mes)
{
  bool rc = false;
  if (strlen(mes) < MAX_NUMBER_LENGTH)
  {
    strcpy(mesbuff, mes);
...
int PDU::convert_ascii_to_7bit(char *ascii, char *a7bit) {
  int r;
  int w;
...
int PDU::ascii_to_pdu(char *ascii, uint8_t *pdu)
{
  int r;
  int w;
  char ascii7bit[70];
...
int PDU::encodePDU(uint8_t *pdubuffer)
{
  ...
  if (recvalid && mesvalid && csvalid)
  {
    ...
    pdubuffer[index++] = reclength;
    pdubuffer[index++] = *recbuff == '+' ? INTERNATIONAL_NUMBER : NATIONAL_NUMBER;
    // now convert recipient to nibble swapped BCD format
    if (*recbuff == '+')
      sourceIndex++;
    stringToBDC(&recbuff[sourceIndex], &pdubuffer[index]);
    index += (reclength + 1) / 2;
    // PID
    pdubuffer[index++] = 0;
    // DCS
...
int PDU::encodePDU(uint8_t *pdubuffer, char *recipient, char *message, enum eDCS dcs)
{
  int length = -1;
  bool rc = setRecepient(recipient);
...
int PDU::encodePDU(uint8_t *pdubuffer, char *recipient, char *message)
{
  return encodePDU(pdubuffer, recipient, message, ALPHABET_7BIT);
}

uint8_t PDU::gethex(char *pc)
{
  int i;
  if (isdigit(*pc))
    i = (uint8_t(*pc) - '0') * 16;
...
int PDU::pdu_to_ascii(char *pdu, int pdulength, char *ascii) {
  int   r;
  int   w;
  int   length;
...
bool PDU::decodePDU(char *pdu, char *SCA, char *ts, char *sender, char *text)
{
  bool rc = true;
  int index = 0;
  int outindex = 0;
  ...
  if (scalength > 0)
  {
    ...
    index += 2;
    if (X = 0x91)
      SCA[outindex++] = '+';   // international phone number
                   // decode BCD
    scalength--;  // includes number type
    for (i = 0; i < scalength; i++)
    {
      // only decimal digits here
      // low uint8_t
      X = gethex(&pdu[index]);
      index += 2;
      int nibble = X & 0xf;
      if (nibble <= 9)
        SCA[outindex++] = nibble + 0x30;
      nibble = X >> 4;
      if (nibble <= 9)
        SCA[outindex++] = nibble + 0x30;
    }
    SCA[outindex] = '0';  // end marker
  }
  X = gethex(&pdu[index]);
  index += 2;
//  printf("PDU %02X\n", X);
  ...
  for (i = 0; i < spl; i += 2)
  {
    X = gethex(&pdu[index]);
    index += 2;
    sender[outindex++] = (X & 0xf) + 0x30;
    sender[outindex++] = (X >> 4) + 0x30;
  }
  sender[outindex++] = 0;
...
//  printf("DCS %02X\n", dcs);
  ...
  for (i = 0; i < 7; i++)
  {
    X = gethex(&pdu[index]);
    index += 2;
    ts[outindex++] = (X & 0xf) + 0x30;
    ts[outindex++] = (X >> 4) + 0x30;
  }
  ts[outindex] = 0;
  ...
  case DCS_7BIT_ALPHABET_MASK:
    ...
    dulength = X;
    outindex = 0;
    i = pdu_to_ascii(&pdu[index], dulength, text);
    text[i] = 0;
    break;
  case DCS_8BIT_ALPHABET_MASK:
    strcpy(text, "Cannot handle 8 bit");
    rc = false;
    break;
  case DCS_16BIT_ALPHABET_MASK:
    strcpy(text, "Cannot handle 16 bit");
    rc = false;
    break;
  }
  return rc;
