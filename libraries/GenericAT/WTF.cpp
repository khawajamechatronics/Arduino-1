...
WTF::WTF(SoftwareSerial &stream, int timeout) :
...
boolean  WTF::find(char *target)
{
     return findUntil(target, NULL);
}

// as above but search ends if the terminate string is found
boolean  WTF::findUntil(char *target, char *terminate)
{
...
int WTF::getString( char *pre_string, char *post_string, char *buffer, int length)
{
     if( find(pre_string) ) {
          int index = 0;
          *buffer = 0;
...
long WTF::getValue(char skipChar)
{
...
// this allows format characters (typically commas) in values to be ignored
float WTF::getFloat(char skipChar)
{
     boolean isNegative = false;
...
unsigned long WTF::getTimeout()
{
     return timeout;
}

// set a new value for the wait timeout
void WTF::setTimeout(unsigned long timeout)
{
     this->timeout = timeout * 1000L;
}
