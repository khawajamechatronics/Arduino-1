...
an example of usage:
        GSM atdevice;
        atdevice.SendSMS("00XXXYYYYYYYYY", "SMS text");
**********************************************************/
char smsGSM::SendSMS(char *number_str, char *message_str) 
{
  if(strlen(message_str)>159)
	DebugPrint(F("Don't send message longer than 160 characters"));
  char ret_val = -1;
  byte i;
  char end[2];
  end[0]=0x1a;
...
/*
  if (CLS_FREE != atdevice.GetCommLineStatus()) return (ret_val);
...
*/
  ...
  for (i = 0; i < 1; i++) {
    ...
    atdevice.SimpleWrite(S_SMS_SEND);
    atdevice.SimpleWrite(number_str);  
    atdevice.SimpleWriteln("\"");
    
	#ifdef DEBUG_ON
		DebugPrintln("DEBUG:SMS TEST");
	#endif
    // 1000 msec. for initial comm tmout
    // 50 msec. for inter character timeout
    if (RX_FINISHED_STR_RECV == atdevice.WaitResp(1000, 500, ">")) {
		#ifdef DEBUG_ON
			DebugPrintln("DEBUG:>");
		#endif
      // send SMS text
      atdevice.SimpleWrite(message_str); 
      atdevice.SimpleWriteln(end);
	  //_cell.flush(); // erase rx circular buffer
      if (RX_FINISHED_STR_RECV == atdevice.WaitResp(7000, 5000, S_SMS_SEND_RESPONSE)) {
...
char smsGSM::SendSMS(byte sim_phonebook_position, char *message_str) 
{
  ...
  if (1 == atdevice.GetPhoneNumber(sim_phonebook_position, sim_phone_number)) {
...
an example of use:
        ...
        position = atdevice.IsSMSPresent(SMS_UNREAD);
        if (position) {
          // read new SMS
          atdevice.GetSMS(position, phone_num, sms_text, 100);
...
char smsGSM::IsSMSPresent(byte required_status) 
{
  ...
  switch (required_status) {
    case SMS_UNREAD:
      atdevice.SimpleWriteln(F("AT+CMGL=\"REC UNREAD\""));
      break;
    case SMS_READ:
      atdevice.SimpleWriteln(F("AT+CMGL=\"REC READ\""));
      break;
    case SMS_ALL:
      atdevice.SimpleWriteln(F("AT+CMGL=\"ALL\""));
      break;
  }
  ...
  switch (status) {
    ...
    case RX_FINISHED:
      ...
      // here we have atdevice.WaitResp() just for generation tmout 20msec. in case OK was detected
      // not due to receiving
      atdevice.WaitResp(20, 20); 
      break;
  }
...
an example of usage:
        ...
        char timestamp[20]; // array for the timestamp string
        char sms_text[100]; // array for the SMS text string

        position = atdevice.IsSMSPresent(SMS_UNREAD);
        if (position) {
          // there is new SMS => read it
          atdevice.GetSMS(position, phone_num, timestamp,sms_text, 100);
          #ifdef DEBUG_PRINT
            atdevice.DebugPrint("DEBUG SMS phone number: ", 0);
            atdevice.DebugPrint(phone_num, 0);
            atdevice.DebugPrint("timestamp: ", 0);
            atdevice.DebugPrint(timestamp, 0);
            atdevice.DebugPrint("\r\n          SMS text: ", 0);
            atdevice.DebugPrint(sms_text, 1);
          #endif
        }        
**********************************************************/
char smsGSM::GetSMS(byte position, char *phone_number, char *timestamp,char *SMS_text, byte max_SMS_len) 
{
  char ret_val = -1;
  char *p_char; 
  char *p_char1;
  ...
  atdevice.SimpleWrite(F("AT+CMGR="));
  atdevice.SimpleWriteln((int)position);  

  // 5000 msec. for initial comm tmout
  // 100 msec. for inter character tmout
  switch (atdevice.WaitResp(5000, 100, "+CMGR")) {
    case RX_TMOUT_ERR:
      // response was not received in specific time
      ret_val = -2;
      break;
    ...
    case RX_FINISHED_STR_NOT_RECV:
      ...
      else if(atdevice.IsStringReceived("ERROR")) {
    ...
    case RX_FINISHED_STR_RECV:
      ...
      if(atdevice.IsStringReceived("\"REC UNREAD\"")) { 
      ...
      //<CR><LF>+CMGR: "REC READ","+XXXXXXXXXXXX",,"02/03/18,09:54:28+40"<CR><LF>
       ...
       //<CR><LF>+CMGR: "REC READ","+972545919886","","16/07/01,17:48:55+12"<CR><LF>
       //Vzsdne
     else if(atdevice.IsStringReceived("\"REC READ\"")) {
      ...
      // ---------------------------
	DebugPrint("DEBUG:SMS getSMS");
	DebugPrint((char *)&atdevice.comm_buf);
	...
	//	 DebugPrint(p_char);
			...
			if (p_char == NULL)
				strcpy(phone_number,"no end");
			else
			{
				*p_char = 0; // end of string
				DebugPrint(p_char1);
			//	strcpy(phone_number, (char *)(p_char1));
				strcpy(phone_number, "0545919886");
			//	memcpy(phone_number,p_char1,strlen(p_char1)+1);
				DebugPrint(phone_number);
			}		  
	}
  	  // extract timestamp
	  // pchar point to end of phone number
//	  p_char1 = strchr(p_char+1,'"'); // beginning of timestamp
	p_char1 = p_char + 6; // beginning of timestamp
      ...
      if (p_char != NULL) {
        ...
        // directly in the atdevice.WaitResp() routine
...
an example of usage:
        ...
        if (GETSMS_AUTH_SMS == atdevice.GetAuthorizedSMS(1, phone_num, sms_text, 100, 1, 3)) {
        ...
        if (GETSMS_AUTH_SMS == atdevice.GetAuthorizedSMS(1, phone_num, sms_text, 100, 0, 0)) {
                           ...
                           byte first_authorized_pos, byte last_authorized_pos)
{
  ...
  byte i;
  char ts[20];
  ret_val = GetSMS(position, phone_number, ts, SMS_text, max_SMS_len);
  if (ret_val < 0) {
...
char smsGSM::DeleteSMS(byte position) 
{
  ...
  atdevice.SimpleWrite(F("AT+CMGD="));
  atdevice.SimpleWriteln((int)position);  
...
/*
	mode true - set TEXT mode
	mode false - set PDU mode
*/
bool smsGSM::setSMStextMode(bool mode)
{
	atdevice.SimpleWriteln(mode ? S_SMS_TEXT_MODE : S_SMS_PDU_MODE);
	atdevice.WaitResp(1000, 1000);
}
