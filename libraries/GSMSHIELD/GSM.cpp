...
int GSM::begin(long baud_rate){
	#ifdef UNO
		if (baud_rate==115200){
			DEBUG_SERIAL.println("Don't use baudrate 115200 with Software DEBUG_SERIAL.\nAutomatically changed at 9600.");
			baud_rate=9600;
		}
	#endif
	int response=-1;
	...
	for (cont=0; cont<3; cont++){
	    ...
	    // there is no response => turn on the module
			#ifdef DEBUG_ON
				DEBUG_SERIAL.println("DB:NO RESP");
			#endif
...
byte GSM::WaitResp(unsigned long start_comm_tmout, unsigned long max_interchar_tmout, 
...
char GSM::SendATCmdWaitResp(char const *AT_cmd_string,
                unsigned long start_comm_tmout, unsigned long max_interchar_tmout,
...
<			DEBUG_SERIAL.print((unsigned long)(millis() - prev_time));	
	...
	#endif
	*/
    if ((millis() - prev_time) >= interchar_tmout) {
	...
	*/
	...
	else
	{
	  ...
	  */
		...
		DEBUG_SERIAL.print("ATT: ");
		DEBUG_SERIAL.println(compare_string);
		DEBUG_SERIAL.print("RIC: NO STRING RCVD");
	#endif
  }
...
void GSM::RxInit(unsigned long start_comm_tmout, unsigned long max_interchar_tmout)
{
