...
class GSM
{
  ...
  private:
    ...
    unsigned long start_reception_tmout; // max tmout for starting reception
    unsigned long interchar_tmout;       // previous time in msec.
  ...
  public:
    ...
    void RxInit(uint32_t start_comm_tmout, uint32_t max_interchar_tmout);
    byte IsRxFinished(void);
    byte IsStringReceived(char const *compare_string);
    byte WaitResp(unsigned long start_comm_tmout, unsigned long max_interchar_tmout);
    byte WaitResp(unsigned long start_comm_tmout, unsigned long max_interchar_tmout, 
