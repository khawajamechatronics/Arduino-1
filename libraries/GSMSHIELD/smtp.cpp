...
/*
   s is a datetime string in the format YY/mm/dd,hh:mm:SS+tz
   where +tz is difference from GMT in 1/4 hours
   e.g
   THis is same as timestamp format in middle of SMS message, 
*/
...
char *SMTPGSM::SmtpGetClock()
{
	...
	if (writewithtimeout(printbuf,OK,1000,150))
	{
		...
		char *start = strstr((char *)gsm.comm_buf, "+CCLK:");
		start += 6; // skip over
